...
export async function createTableDynamo() {
  ...
  let nameTable = await prompt.question(
    "### Por favor ingrese el nombre de la tabla ####\n"
  );
  prompt.close();
  ...
  if (nameTable != "") {
    ...
    if (response.$metadata.httpStatusCode == 200) {
      ...
      console.log(
        `hubo un error al momento de crear la tabla , codigo de la peticion http ${response.$metadata.httpStatusCode}`
      );
    }
  }
}
...
export async function deleteTableDynamo() {
  ...
  let nameTable = await prompt.question(
    "### Por favor ingrese el nombre de la tabla a borrar ####\n"
  );
  prompt.close();
  ...
  if (exist) {
    ...
    console.log(
      "respuesta de  la peticion : " + response.$metadata.httpStatusCode
    );
    console.log(
      `se a eliminado la tabla con el id ${response.TableDescription.TableId} exitosamente !!!`
    );
  } else {
